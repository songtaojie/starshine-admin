	...
	<div class="sys-oplog-container" v-loading="state.loading">
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 8px">
			<el-table :data="state.logData" @sort-change="sortChange" style="width: 100%" border :row-class-name="tableRowClassName">
				...
				<el-table-column prop="displayName" label="显示名称" width="170" header-align="center" show-overflow-tooltip />
				...
				<el-table-column prop="logLevel" label="级别" width="70" align="center" show-overflow-tooltip>
					<template #default="scope">
						<el-tag v-if="scope.row.logLevel === 0">详细</el-tag>
...
const handleQuery = async () => {
	...
	var res = await getAPI(SysLogApi).getOpLogPage(params);
...
const clearLog = async () => {
	state.loading = true;
	await getAPI(SysLogApi).clearOpLog();
	state.loading = false;
